...
import warnings
from fluvius_session.base import BaseSessionInterface
...
class MongoDBSessionInterface(BaseSessionInterface):
    ...
    def __init__(
        ...
        if _SessionModel is None:
            raise RuntimeError(
                "Please install Mongo dependencies: " "pip install fluvius_session[mongo]"
            )
    ...
    async def _set_value(self, key, data):
        ...
        await _SessionModel.replace_one(
            {self.sid_field: key},
            {self.sid_field: key, self.expiry_field: expiry, self.data_field: data},
            upsert=True,
        )
