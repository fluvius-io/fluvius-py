...
from fluvius_session.utils import CallbackDict
...
class BaseSessionInterface(metaclass=abc.ABCMeta):
    ...
    def __init__(self, expiry, prefix, cookie_name, domain, httponly,
                 sessioncookie, samesite, session_name, secure):
        self.expiry = expiry
        self.prefix = prefix
    ...
    def _delete_cookie(self, request, response):
        ...
        response.cookies[self.cookie_name]['expires'] = datetime.fromtimestamp(0)
    ...
    async def save(self, request, response) -> None:
        ...
        if req[self.session_name].modified:
            val = self._pack(req[self.session_name])
