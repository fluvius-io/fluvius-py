...
from fluvius.dmap.processor.manager import DataProcessManager, PostgresFileProcessManager
...
def manager_wrapper(func):
    ...
    def _file_parser(
        ...
        def register_process_manager():
            if not getattr(process_config, 'manager', None):
                return no_op, file_resource

            pm = DataProcessManager.init_manager(process_config.manager.name, config=process_config.manager)
            rc = process_config.reader
            file_processor_handler = pm.register_file(
                file_resource,
                data_provider=rc['name'],
                data_variant=rc.get('variant', None),
                status="RUNNING",
            ...
            return status_setter(file_processor_handler), file_resource.set(
                'source_id', file_processor_handler._id
            )

        filepath = file_resource.filepath
        try:
            set_status, file_resource = register_process_manager()
...
def process_input(process_config: DataProcessConfig, *args):
    ...
    @manager_wrapper
    def process_pipelines(file_resource):
