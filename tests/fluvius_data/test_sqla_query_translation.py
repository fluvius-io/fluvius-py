...
class TestSqlaDriver(SqlaDriver):
    __db_dsn__ = "sqlite+aiosqlite:///:memory:" # In-memory for testing

    def compile_statement(self, stmt):
        return str(stmt.compile(dialect=sqlite.dialect(), compile_kwargs={"literal_binds": True})).replace('\n', '')

# --- Test Schemas (using local base) ---
BaseTestSchema = TestSqlaDriver.__data_schema_base__
...
def test_build_select_simple(test_driver):
    ...
    expected_sql = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt) == expected_sql

def test_build_select_with_specific_fields(test_driver):
    query = BackendQuery.create(select=['_id', 'name', 'email'])
    stmt = test_driver.build_select(UserSchema, query)
    expected_sql = "SELECT user._id, user.name, user.email FROM user LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt) == expected_sql
...
def test_build_select_with_identifier(test_driver):
    ...
    stmt = test_driver.build_select(UserSchema, query)
    expected_sql = f"SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE user._id = '{user_id}' LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt) == expected_sql

def test_build_select_with_where_eq(test_driver):
    query = BackendQuery.create(where={"name": "John Doe"})
    stmt = test_driver.build_select(UserSchema, query)
    expected_sql = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE user.name = 'John Doe' LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt) == expected_sql
...
def test_build_select_with_where_gt_lt_gte_lte(test_driver):
    ...
    stmt = test_driver.build_select(UserSchema, query)
    sql = test_driver.compile_statement(stmt)
    assert "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE user.age > 30 AND user.age <= 40 LIMIT 100 OFFSET 0" == sql
...
def test_build_select_with_where_in_notin(test_driver):
    ...
    expected_sql_in = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE user.name IN ('Alice', 'Bob') LIMIT 100 OFFSET 0"
    ...
    expected_sql_notin = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE (user.name NOT IN ('Alice', 'Bob')) LIMIT 100 OFFSET 0"
...
def test_build_select_with_where_and_or(test_driver):
    ...
    expected_sql_and = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE user.name = 'John' AND user.age > 25 LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt_and) == expected_sql_and

    query_or = BackendQuery.create(where={".or": [{"name": "Jane"}, {"age.lt": 20}]})
    stmt_or = test_driver.build_select(UserSchema, query_or)
    expected_sql_or = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE user.name = 'Jane' OR user.age < 20 LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt_or) == expected_sql_or
...
def test_build_select_with_where_negated_and_or(test_driver):
    ...
    expected_sql_nand = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE NOT (user.name = 'John' AND user.age > 25) LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt_nand) == expected_sql_nand

    query_nor = BackendQuery.create(where={"!or": [{"name": "Jane"}, {"age.lt": 20}]})
    stmt_nor = test_driver.build_select(UserSchema, query_nor)
    expected_sql_nor = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE NOT (user.name = 'Jane' OR user.age < 20) LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt_nor) == expected_sql_nor
    
def test_build_select_with_limit_offset(test_driver):
    query = BackendQuery.create(limit=10, offset=5)
    stmt = test_driver.build_select(UserSchema, query)
    expected_sql = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user LIMIT 10 OFFSET 5"
    assert test_driver.compile_statement(stmt) == expected_sql
...
def test_build_select_with_sort(test_driver):
    ...
    expected_sql_asc = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user ORDER BY user.name ASC LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt_asc) == expected_sql_asc
    
    query_desc = BackendQuery.create(sort=['name.desc'])
    stmt_desc = test_driver.build_select(UserSchema, query_desc)
    expected_sql_desc = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user ORDER BY user.name DESC LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt_desc) == expected_sql_desc
    ...
    expected_sql_multi_sort = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user ORDER BY user.age DESC, user.name ASC LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt_multi_sort) == expected_sql_multi_sort
...
def test_build_select_with_scope(test_driver):
    ...
    stmt = test_driver.build_select(UserSchema, query)
    expected_sql = "SELECT user._id, user.name, user.age, user.email, user.is_active, user.created_at FROM user WHERE user.is_active = 1 LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt) == expected_sql
...
def test_build_select_with_field_mapping(test_driver):
    ...
    expected_sql = "SELECT user.name AS user_name, user.age AS user_age FROM user WHERE user.name = 'Mapped Name' LIMIT 100 OFFSET 0"
    assert test_driver.compile_statement(stmt) == expected_sql
